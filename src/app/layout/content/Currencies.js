...
export class Currencies extends Component {
  ...
  fetchCurrencies = () => {
    fetch(
      `https://api.coinmarketcap.com/v1/ticker/?convert=${this.state.currencyType.toUpperCase()}&limit=52`
    )
      .then(data => data.json())
      .then(data => {
  ...
  handleConvertCurrencies = (e, currency) => {
    ...
    fetch(
      `https://api.coinmarketcap.com/v1/ticker/?convert=${currency.toUpperCase()}&limit=52`
    )
      .then(data => data.json())
      .then(data => {
